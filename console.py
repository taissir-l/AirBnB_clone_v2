...
class HBNBCommand(cmd.Cmd):
    ...
    def preloop(self):
        """tests if isatty is false
        """
        if not sys.__stdin__.isatty():
            print('(hbnb)')

    def precmd(self, line):
        """command line for advanced command syntax.
    ...
    def do_quit(self, command):
        """exit the RBNB console
        """
        exit(0)

    def help_quit(self):
        """the help documentation to quit
        """
    ...
    def help_destroy(self):
        """info for the destroy command
        """
        print("Destroys an individual instance of a class")
        print("[Usage]: destroy <className> <objectId>\n")

    def do_all(self, args):
        """all objects, or all objects of a class
        """
        print_list = []
        if args:
            args = args.split(' ')[0]
